...
SimpleGraph = function(elemid, data, options) {
  ...
  if (istxtdata==true) {
    ...
    //     }
    //   });  
    //   var sigma = numeric.div(numeric.dot(numeric.transpose(A), A), A.length);
    //   var ans = numeric.svd(sigma);
    //   d["theta"] = Math.atan2(ans.U[1][0],ans.U[0][0])*180/Math.PI;
    //   d["r"] = {"x":Math.sqrt(ans.S[0]) , "y":Math.sqrt(ans.S[1])};
    // });
...
SimpleGraph.prototype.update = function() {
  ...
  circle.enter().append("circle")
      ...
      .on("dblclick", function(d){dblclick(d)}).on("mousedown.drag",  self.datapoint_drag())
  ...
  if (istxtdata==true) {
    ...
    // var ellip = this.vis.select("svg").selectAll("ellipse")
    //     .data(this.cents);

    // ellip.enter().append("ellipse")
    //     .attr("cx",    function(d) { return self.x(d["values"].x); })
    //     .attr("cy",    function(d) { return self.y(d["values"].y); })
    //     .style("fill", "none")
    //     .style("stroke-width", 1.5)
    //     .style("stroke", function(d) { return color(d["key"]); });

    // ellip.attr("cx",    function(d) { return self.x(d["values"].x); })
    //     .attr("cy",    function(d) { return self.y(d["values"].y); })
    //     .attr("rx",    function(d) { return self.x(d["r"]["x"])-self.x(0); })
    //     // .attr("rx",    function(d) { return d["r"]["x"]; })
    //     .attr("ry",    function(d) { return self.y(0) - self.y(d["r"]["y"]); })
    //     // .attr("ry",    function(d) { return d["r"]["y"];; })
    //     .attr("transform", function(d) { return "rotate(" +d["theta"]+","+self.x(d["values"].x)+","+self.y(d["values"].y)+")"; })
    //     .style("fill", "none")
    //     .style("stroke-width", 1.5)
    //     .style("stroke", function(d) { return color(d["key"]); });
...
dblclick = function(d) {
  ...
  //   //   console.log(attr[i]);
  //   //   if (Math.abs(x0[attr[i]]-x1[attr[i]]) > max) {
  //   //     max = Math.abs(x0[attr[i]]-x1[attr[i]]);
  //   //     index = i;
  //   //     console.log("-----------------");
  //   //   }
  //   // }
  //   // var V = jQuery.extend(true, {}, x0);
  //   // for (var i = 0; i<attr.length-2; i++) {
  //   //   if (i==index) {
  //   //     V[attr[i]] = 1;
  //   //   }
  ...
  //   }
  //   for (var i = 0; i<attrNo; i++) {
  //    V[attr[i]] = x0["coord"][attr[i]]-x1["coord"][attr[i]];
  //    // norm = norm + Math.pow(x0[attr[i]]-x1[attr[i]],2);
  //    norm = norm + (x0["coord"][attr[i]]-x1["coord"][attr[i]])*(x0["coord"][attr[i]]-x1["coord"][attr[i]]);
  //    // console.log(norm)
  //   }
  //   norm = Math.sqrt(norm);
  //   for (var i = 0; i<attrNo; i++) {
  ...
  //     }
  //     d["coord"][newxname] = Math.log(d["coord"][newxname]); 
  //   });
  ...
  //   d3.select("#SC").remove();
  //   if (graph.options.alt == "x") {
  //     d3.select("#X").remove();
  //     data.forEach(function(d) {d.x = d["coord"][newxname]; });
  //     graph = new SimpleGraph("scplot", data, {
  //           "xlabel": newxname,
  //           "ylabel": graph.options.ylabel,
  //           "alt": "y",
  //           "init": false
  //         });
  //     for (var i = 0; i<attrNo; i++) {
  //       X[i] = {"attr":attr[i], "value":V[attr[i]]};
  //     }
  //     xaxis = new Xaxis("#scplot", X, {
  ...
  //       });
  //     attr2[attr2.length] = {"attr": newxname, "vector": X};
  //     document.getElementById("cbX").selectedIndex = attrNo+index-1;
  //   }
  //   else {
  //     d3.select("#Y").remove();
  //     data.forEach(function(d) {d.y = d["coord"][newxname]; });
  //     graph = new SimpleGraph("scplot", data, {
  //           "xlabel": graph.options.xlabel,
  //           "ylabel": newxname,
  //           "alt": "x",
  //           "init": false
  //         });
  //     for (var i = 0; i<attrNo; i++) {
  //       Y[i] = {"attr":attr[i], "value":V[attr[i]]};
  //     }
  //     yaxis = new Yaxis("#scplot", Y, {
  ...
  //       });
  //     attr2[attr2.length] = {"attr": newxname, "vector": Y};
  //     document.getElementById("cbY").selectedIndex = attrNo+index-1;
  //   }
...
SimpleGraph.prototype.datapoint_drag = function() {
  ...
  return function(d) {
    ...
    self.update();
    // d3.select("#SC").select("svg").append("circle")
    //   .attr("cx",    30)
    //   .attr("cy",    30)
    //   .attr("r", 50.0);
  }
...
updategraph = function(axistobeupdated,givenV,givenVchanged) {
  ...
  if (givenV == undefined) {
    ...
    graph.points.forEach(function(d,i) {
      ...
      // d["coord"][newxname] = d["coord"][newxname]==0 ? 0 : Math.sign(d["coord"][newxname])*Math.log(Math.abs(d["coord"][newxname])); 
      if (istxtdata==true) {
        d["coord"][newxname] = d["coord"][newxname]==0 ? 0 : Math.sign(d["coord"][newxname])*Math.pow(Math.abs(d["coord"][newxname]),0.5);
        // d["coord"][newxname] = d["coord"][newxname]==0 ? 0 : Math.sign(d["coord"][newxname])*Math.log(Math.abs(d["coord"][newxname])+1);
      }
    });
    // d3.select("#cbY").append("option").attr("value",newxname).text(newxname);
    // d3.select("#cbX").append("option").attr("value",newxname).text(newxname);
